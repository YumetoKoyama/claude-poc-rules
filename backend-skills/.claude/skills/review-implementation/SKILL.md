...
1. **iteration を取得**（rules 改善・判定の独立性）: `bash .claude/skills/_common/scripts/get-review-iteration.sh implement` を実行し、stdout の数値を `N` とする。出力ファイル名を `round-<N>-review.json` とする。**state.json を直接 Read してはならない**（produce 段の判断に引きずられないため）。
2. **差分の特定**: `git diff --name-only main...HEAD` で変更ファイル一覧を取得。
3. **関連設計書を特定**: state または Issue 本文から SCR-XXX / API 名 / テーブル名を抽出し、`docs/design/` の該当ファイルを Read。**あわせて横断設計書 `docs/design/共通部品設計.md` を必ず Read し、定義済み共通部品（GlobalExceptionHandler / 共通バリデーション / 共通レスポンス整形 / ロギング / `JwtUtil` 等）の一覧を把握する**（`common_component` 観点の照合先。再発明があれば BLOCK）。
4. **品質ゲート結果の確認（内容依存判定・RC-07・収束不能バグ修正）**: レポートの**更新時刻にもコミットハッシュにも依存しない**。代わりに `bash .claude/skills/_common/scripts/gate-content-hash.sh` を**再計算**し、`implement-from-issue` / `fix-implementation` がゲート実行直後に保存した `target/.gate-content`（コード内容ハッシュ）と照合して、**現在のコードに対して実際に実行されたレポートか**を判定する。一致すれば有効。`docs(review)` 等のコードを変えないコミットではハッシュが変化しないため、旧 `.gate-commit==HEAD` 方式で起きていた誤検知（HEAD が進むたびに `quality_gate` が立ち、ループが収束しない）は発生しない。**`.gate-content` が存在しない移行期のレポートに限り、従来の `.gate-commit` / 更新時刻でフォールバック判定してよい**。いずれでも実行が確認できなければ category=`quality_gate` の BLOCK とする:
...
- `security`: OWASP ベースの脆弱性点検（PR 作成前に必須）。以下を確認し未対応があれば BLOCK:
  - SQL インジェクション・XSS・**認可バイパス / IDOR・テナント越境**・JWT 検証漏れ（署名・失効・有効期限）・ハードコードされたシークレット
  - **PII / 機密情報のログ出力**（パスワード・メールアドレス・電話番号・トークンを `log.*` に直接渡していないか。R-SEC-050〜051）
  - **Jackson デシリアライズ安全**（`enableDefaultTyping` / `JsonTypeInfo.Id.CLASS` の使用禁止。R-SEC-070〜071）
  - **CORS ワイルドカード禁止**（`allowedOrigins("*")` が無いこと。R-SEC-091）・Spring Security デフォルトヘッダの無効化禁止（R-SEC-092）
  - **TOCTOU 防止**：認可チェックとデータ操作が同一 `@Transactional` 境界内にあること（R-SEC-100）
  - **機密フィールドの Response 混入禁止**（Response DTO にパスワード・トークン等が含まれていないこと。R-SEC-110〜111）
  - `docs/design/セキュリティテスト観点.md` の観点と対応づける
  - **SQL インジェクション**：`@Query` 内の文字列連結・`createNativeQuery` での動的組み立て・ユーザー入力をカラム名に直接使用していないこと（R-SEC-001〜003）
  - **認証実装安全**：JWT を Authorization ヘッダから手動パースしていないこと・シークレットのハードコードがないこと・パスワードが BCrypt でハッシュされていること（R-SEC-031〜033）
  - **`augment-generated-models` の補完チェックリスト**（`@PreAuthorize` 付与・テナント越境防止・UseCase Validator・PII ログマスク・Jackson 安全設定・CORS・機密 DTO・SQL インジェクション・認証実装安全 等）の各項目が充足されているかを差分で確認し、いずれかが未対応なら BLOCK とする
- `architecture`: Controller に業務ロジック、フロントに業務判定、REST 以外の画面描画、`.env` の直接コミット
- `common_component`: `docs/design/共通部品設計.md` に定義済みの共通部品（例外ハンドラ・共通バリデーション・レスポンス整形・ロギング・`JwtUtil` 等）を再実装している（再発明。既存部品を再利用すべき）
...
- `security_suggest`: 以下はルール上「推奨」のため SUGGEST とする
  - 外部公開 ID（API レスポンスの `id` フィールド等）が連番の場合、UUID への変更を推奨（R-SEC-012）
  - P0 機能（状態遷移・金額変更）で `@Version` による楽観ロックが使われていない場合、競合リスクを指摘（R-SEC-101）
