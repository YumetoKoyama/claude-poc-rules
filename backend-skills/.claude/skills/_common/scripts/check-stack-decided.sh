...
#       - 親アンブレラから実行: claude-poc-frontend / claude-poc-backend / claude-poc-batch の確定表を検査
...
if compgen -G "$ROOT/.claude/rules/"*-00-stack.md > /dev/null 2>&1; then
  ...
  # 親アンブレラ
  # 実装を伴う子（実装コードを持つリポジトリ）を対象に確定表を検査する。
  # claude-poc-batch も実装を伴うため対象に含める（RC-09 / D-15）。
  for child in claude-poc-frontend claude-poc-backend claude-poc-batch; do
  ...
  # ドキュメント専用リポジトリ（claude-poc-docs 等）からの呼び出しに対応:
  # 子が見つからない場合、1 階層上を試みる
  if [[ ${#targets[@]} -eq 0 && ${#missing[@]} -gt 0 ]]; then
    root_abs="$(cd "$ROOT" && pwd)"
    set +e
    parent_dir="$(cd "$ROOT/.." && pwd 2>/dev/null)"
    set -e
    if [[ -n "$parent_dir" && "$parent_dir" != "$root_abs" ]]; then
      missing=()
      for child in claude-poc-frontend claude-poc-backend claude-poc-batch; do
        expected="$parent_dir/$child/.claude/rules"
        found=0
        if compgen -G "$expected/"*-00-stack.md > /dev/null 2>&1; then
          while IFS= read -r f; do targets+=("$f"); done < <(ls "$expected/"*-00-stack.md)
          found=1
