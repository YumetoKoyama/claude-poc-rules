...
if [[ "$PHASE" == "unit" ]]; then
  ...
  if [[ ! -f "$IT_MATRIX" ]]; then
    ...
    it_count="$(count_pat 'IT-[0-9]{3}' "$IT_MATRIX")"
    if [[ "$it_count" -ge 1 ]]; then
      echo "OK: $IT_MATRIX（IT ${it_count} 件）"
    elif grep -Eq '対象外|後続|N/?A|なし|該当なし' "$IT_MATRIX"; then
      echo "OK: $IT_MATRIX（IT 行なし・対象外理由の明記を確認）"
    else
...
if [[ ! -f "$RTM" ]]; then
  ...
  if [[ -n "$ISSUE" ]]; then
    ...
    rtm_tc="$(count_pat '(TC|IT)-[0-9]{3}' "$RTM")"
    if [[ "$rtm_tc" -lt 1 ]]; then
